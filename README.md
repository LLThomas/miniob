...
| <ul><li>[ ] thomas </li></ul> | 唯一索引           | 10   | 唯一索引：create unique index                                | create unique index i_id on t1(id);<br/>insert into t1 values(1,1);<br/>insert into t1 values(1,2); -- failed |
| <ul><li>[ ] huaouo </li></ul> | 支持NULL类型       | 10   | 包括但不限于建表、查询和插入。默认情况不允许为NULL，使用nullable关键字表示字段允许为NULL。<br/>Null不区分大小写 | create table t1 (id int not null, age int not null, address nullable); create table t1 (id int, age int, address char nullable); insert into t1 values(1,1, null); |
...
| <ul><li>[ ] thomas </li></ul> | 多列索引           | 20   | 单个索引关联了多个字段                                       | create index i_id on t1(id, age);                            |
