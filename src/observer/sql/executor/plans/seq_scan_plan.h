 ...
 public:
  SeqScanPlanNode(TupleSchema* output, const char* table_name,
      ...
      : AbstractPlanNode(output, {}),
        ...
        condition_filters_(condition_filters_) {}
  SeqScanPlanNode(TupleSchema* output, const char* table_name,
  ...
  std::vector<const AbstractExpression*> GetPredicates() { return predicates_; }

 private:
  const char* table_name_;  //需要进行序列扫描的表名（或唯一标识符）
  std::vector<DefaultConditionFilter*> condition_filters_;
  std::vector<const AbstractExpression*> predicates_;
};
#endif