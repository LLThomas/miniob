...
void LockTrace::foundDeadLock(LockID &current, LockTrace::LockID &other,
  ...
  if (itLocks == mLocks.end()) {
    LOG_ERROR(
        ...
        current.mThreadId, otherWaitMutex, current.mFile.c_str(), current.mLine,
        other.mThreadId, current.mFile.c_str(), current.mLine, otherWaitMutex);
  } else {
    ...
    LOG_ERROR(
        ...
        other.mThreadId, current.mFile.c_str(), current.mLine, otherWaitMutex,
        otherRecusive.mFile.c_str(), otherRecusive.mLine);
  }
}
...
bool LockTrace::checkLockTimes(pthread_mutex_t *mutex, const char *file,
  ...
  if (lockTimes >= mMaxBlockTids) {
    ...
    LOG_WARN(
        ...
        "time:%ld:%s:%d",
        mutex, lockTimes, file, line, lockId.mThreadId, lockId.mFile.c_str(),
        lockId.mLine);
