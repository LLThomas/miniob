...
bool check_directory(std::string &path) {
  ...
  if (0 == mkdir(path.c_str(), 0777) || is_directory(path.c_str())) return true;

  bool sep_state = false;
  for (int i = 0; i < len; i++) {
    if (path[i] != '/') {
      if (sep_state) sep_state = false;
      continue;
    }

    if (sep_state) continue;
...
int list_file(const char *path, const char *filter_pattern,
  ...
  if (!pdir) {
    if (filter_pattern) regfree(&reg);
    LOG_ERROR("open directory failure. path %s, errmsg %d:%s", path, errno,
              strerror(errno));
    closedir(pdir);
    return -1;
  }
  ...
  while ((0 == readdir_r(pdir, &entry, &pentry)) && (NULL != pentry)) {
    ...
    if (is_directory(tmp_path)) continue;
