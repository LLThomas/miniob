...
bool TimerToken::operator<(const TimerToken &other) const {
  if (TIMEVAL_LESS_THAN(time, other.time)) return true;
  if (TIMEVAL_EQUAL(time, other.time)) return (nonce < other.nonce);
  return false;
}
...
void TimerStage::register_timer(TimerRegisterEvent &reg_ev) {
  ...
  if (result.first == timer_queue_.begin()) check_timer = true;
  ...
  if (check_timer) trigger_timer_check();
...
void TimerStage::check_timer() {
  ...
  while (true) {
    ...
    for (last = first; last != timer_queue_.end(); ++last) {
      if (TIMEVAL_LESS_THAN(now, last->first.get_time())) break;
